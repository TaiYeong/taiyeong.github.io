...
- Release 버전으로 빌드하려면 다음 단계를 따르세요. 명령어는 Linux/macOS 기준이며, Windows에서도 비슷하게 적용됩니다(예: Visual Studio 빌드 시스템 사용 시).

### Linux user
CMake를 실행할 때 `-DCMAKE_BUILD_TYPE=Release` 옵션을 추가합니다.
```bash
cmake .. -DCMAKE_BUILD_TYPE=Release
make # make 명령어: 생성된 빌드 시스템을 사용해 실제로 소스코드를 컴파일하고 실행 파일을 만드는 명령어.
```
- `..`: 소스 디렉토리(CMakeLists.txt가 있는 곳)를 가리킵니다.
- `-DCMAKE_BUILD_TYPE=Release`: 빌드 타입을 Release로 설정합니다.
- `make`는 소스 코드를 컴파일해 최적화된 실행 파일을 생성합니다.
- 병렬 빌드를 위해 CPU 코어를 활용하려면 `make -j$(nproc)`를 사용할 수 있습니다. 예: `make -j4` (4코어 사용).

### Windows user (Visual Studio)
Windows에서 Visual Studio를 사용하는 경우, CMake는 Visual Studio 솔루션 파일(.sln)을 생성합니다. 이 경우:
```bash
cmake .. -DCMAKE_BUILD_TYPE=Release
cmake --build . --config Release
```
- `--config Release`: Visual Studio 빌드에서 Release 모드를 지정합니다.

#### Tips
- **확인 방법**: 빌드가 Release 모드로 되었는지 확인하려면, 생성된 실행 파일의 크기가 Debug 모드보다 작거나, 디버깅 심볼이 포함되지 않은 것을 확인할 수 있습니다.
- **CMakeLists.txt에서 기본 설정**: 프로젝트에서 기본 빌드 타입을 설정하려면 CMakeLists.txt에 아래를 추가할 수 있습니다:
  ```cmake
