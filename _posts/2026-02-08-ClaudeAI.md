...
* **서브에이전트 (Subagents) - *Pipeline의 핵심*:**
    ...
    * **토큰 비용 주의:** 메인 AI가 지휘자(Orchestrator) 역할만 수행할 때도 "어떤 에이전트에게 무엇을 시킬지 계획하는 것"만으로 토큰을 소비합니다. 각 서브에이전트는 독립적인 컨텍스트 윈도우를 별도로 생성하므로 비용이 빠르게 누적됩니다. `/cost` 명령어로 수시로 점검하는 것이 권장됩니다.
    * **에이전트가 자동 호출되지 않는 경우:** 단순히 에이전트 파일을 만드는 것만으로는 자동 위임이 보장되지 않습니다. 메인 AI가 판단하기에 스킬(Skills)로 정의된 명확한 워크플로우가 없거나, 직접 처리하는 것이 더 효율적이라고 판단하면 에이전트를 호출하지 않고 스스로 작업을 완료합니다. 에이전트가 예상대로 호출되지 않는다면 스킬 정의 부재가 주된 원인입니다.
...
## Space 03 : English Study Orchestration

## Space 04 : AI Scribe and Note Refiner Orchestration
